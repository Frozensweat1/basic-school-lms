...
class Index extends Component
{
    ...
    public function render()
    {
        ...
        $assignments = $student
            ? Assignment::where('status', 'published')->whereHas('classSubject.schoolClass.academicYear', fn ($query) => $query->where('school_id', $this->parent->school_id))->whereHas('classSubject.schoolClass.enrollments', fn ($query) => $query->where('student_id', $student->id)->where('status', 'active'))->with('classSubject.subject')->latest('due_at')->get()->map(function ($assignment) use ($student) { $assignment->submission = $assignment->submissions()->where('student_id', $student->id)->first(); return $assignment; })
            : collect();
