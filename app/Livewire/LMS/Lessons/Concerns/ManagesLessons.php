...
use App\Models\{Lesson, School, Teacher, Topic};
...
abstract class ManagesLessons extends Component
{
    ...
    public function save(): void
    {
        ...
        try {
            ...
            $data=$this->validate(['topicId'=>['required','integer',Rule::exists('topics','id')],'teacherId'=>['nullable','integer',Rule::exists('teachers','id')],'title'=>['required','string','max:255'],'summary'=>['nullable','string','max:2000'],'content'=>['nullable','string','max:50000'],'objectives'=>['nullable','string','max:5000'],'sequence'=>['required','integer','min:0','max:9999'],'status'=>['required',Rule::in(['draft','published','archived'])]]);
            ...
            Lesson::updateOrCreate(['id'=>$lesson?->id],['topic_id'=>$topic->id,'teacher_id'=>$teacherId,'title'=>$data['title'],'summary'=>$data['summary']?:null,'content'=>$data['content']?:null,'objectives'=>collect(preg_split('/\r\n|\r|\n/',$data['objectives']?:''))->map(fn($item)=>trim($item))->filter()->values()->all()?:null,'sequence'=>$data['sequence'],'status'=>$data['status'],'published_at'=>$data['status']==='published'?($lesson?->published_at??now()):null]);
    ...
    private function resetForm(): void { $this->reset(['editingId','deletingId','topicId','teacherId','title','summary','content','objectives','sequence','status']);$this->sequence='0';$this->status='draft';$this->resetValidation(); }
}
