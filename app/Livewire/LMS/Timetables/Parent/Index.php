...
class Index extends Component
{
    ...
    public function mount(): void
    {
        ...
        $this->parent = $parent;
        $this->studentId = (string) ($this->wardQuery()->value('students.id') ?? '');
    }
    ...
    {
        $students = $this->wardQuery()->get();
        $student = $students->firstWhere('id', (int) $this->studentId);
        $classIds = $student
            ? $student->enrollments()->where('status', 'active')->pluck('school_class_id')
            : collect();
        ...
        $filtered = TimetableEntry::query()
            ...
            ->whereHas('timetable', fn ($query) => $query
                ...
                ->whereHas('academicYear', fn ($year) => $year->where('school_id', $this->parent->school_id)))
        ...
        return view('livewire.lms.timetables.parent.index', [
            'students' => $students,
            ...
            'periods' => SchedulePeriod::query()->where('school_id', $this->parent->school_id)->orderBy('sequence')->get(),
