...
class Index extends Component
{
    ...
    public function mount():void{$this->authorize('viewAny',SchoolClass::class);}
    public function create():void{$this->authorize('create',SchoolClass::class);$this->resetForm();$this->academicYearId=(string)AcademicYear::where('is_active',true)->value('id');$this->showFormModal=true;}
    public function edit(SchoolClass $schoolClass):void{$this->authorize('update',$schoolClass);$this->editingId=$schoolClass->id;$this->academicYearId=(string)$schoolClass->academic_year_id;$this->streamId=(string)($schoolClass->stream_id??'');$this->name=$schoolClass->name;$this->code=$schoolClass->code??'';$this->status=$schoolClass->status;$this->resetValidation();$this->showFormModal=true;}
    public function save():void{
        $class=$this->editingId?SchoolClass::findOrFail($this->editingId):null;$this->authorize($class?'update':'create',$class??SchoolClass::class);
        try{$data=$this->validate(['academicYearId'=>['required','integer','exists:academic_years,id'],'streamId'=>['nullable','integer','exists:streams,id'],'name'=>['required','string','max:100'],'code'=>['nullable','string','max:50'],'status'=>['required','in:active,archived']]);
            ...
            $this->showFormModal=false;$this->resetForm();LivewireAlert::title($class?'Class updated':'Class created')->success()->asToast()->position('top-end')->show();
        }catch(ValidationException $e){LivewireAlert::title('Check the form')->text('Correct the highlighted fields and try again.')->error()->asToast()->position('top-end')->show();throw $e;}catch(Throwable $e){report($e);LivewireAlert::title('Unable to save class')->text('Please try again.')->error()->asToast()->position('top-end')->show();}}
    public function confirmDelete(SchoolClass $schoolClass):void{$this->authorize('delete',$schoolClass);$this->deletingId=$schoolClass->id;$this->showDeleteModal=true;}
