...
class Index extends Component
{
    ...
    public function mount(): void
    {
        ...
        $settings = SchoolSetting::where('school_id', $school->id)->pluck('value', 'key');
    ...
    public function save(): void
    {
        ...
        try {
            ...
            if ($this->logo) $newLogoPath = $this->logo->store('branding', 'public');

            DB::transaction(function () use ($school, $data, $newLogoPath): void {
                $school->update(collect($data)->only(['name', 'code', 'motto', 'email', 'phone', 'address'])->merge(['logo_path' => $newLogoPath])->all());
                foreach ([
            ...
            if ($newLogoPath !== $oldLogoPath && $oldLogoPath) Storage::disk('public')->delete($oldLogoPath);
            $this->logo = null;
