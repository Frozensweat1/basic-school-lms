...
namespace App\Livewire\LMS\Emails;
...
class Index extends Component
{
    ...
    public function mount(EmailRecipientResolver $resolver): void
    {
        $this->authorize('viewAny', EmailCampaign::class);

        $audience = (string) request()->query('audience', '');
        if (in_array($audience, EmailRecipientResolver::AUDIENCES, true)) {
    ...
    public function reviewRecipients(EmailCampaignService $service, ContentSanitizer $sanitizer): void
    {
        $this->authorize('create', EmailCampaign::class);

        try {
            $data = $this->validateComposer();
            $cleanBody = $sanitizer->clean($data['body']);
            if (trim(html_entity_decode(strip_tags(str_replace('&nbsp;', ' ', $cleanBody)))) === '') {
            ...
            if ($resolution['recipients']->isEmpty()) {
                ...
                throw ValidationException::withMessages([
                    $field => $this->mode === 'single'
                        ...
                        : 'No deliverable email addresses match this audience.',
                ]);
            }
            ...
            $this->previewSample = $resolution['recipients']
                ->take(8)
            ...
            LivewireAlert::title('Check the email details')
                ->error()->asToast()->position('top-end')->show();
            throw $exception;
        } catch (Throwable $exception) {
            report($exception);
            LivewireAlert::title('Unable to prepare the recipient list')
                ->error()->asToast()->position('top-end')->show();
        }
    }

    public function queueEmail(EmailCampaignService $service): void
    {
        $this->authorize('create', EmailCampaign::class);
        ...
        try {
            ...
            LivewireAlert::title('Email queued for '.number_format($campaign->recipient_count).' recipient'.($campaign->recipient_count === 1 ? '' : 's'))
                ->success()->asToast()->position('top-end')->show();
        } catch (ValidationException $exception) {
            $this->showReviewModal = false;
            LivewireAlert::title('Recipient details changed — review the email again')
                ->error()->asToast()->position('top-end')->show();
            throw $exception;
        } catch (Throwable $exception) {
            report($exception);
            LivewireAlert::title('Unable to queue the email')
                ->error()->asToast()->position('top-end')->show();
        }
    }
    ...
    public function confirmRetry(int $id): void
    {
        ...
        if ($campaign->failed_count < 1) {
            LivewireAlert::title('There are no failed deliveries to retry')
                ->info()->asToast()->position('top-end')->show();
    ...
    public function retryFailed(EmailCampaignService $service): void
    {
        ...
        try {
            ...
            report($exception);
            LivewireAlert::title('Unable to retry failed deliveries')
                ->error()->asToast()->position('top-end')->show();
        }
    }
    ...
    public function render(EmailRecipientResolver $resolver)
    {
        $schoolId = $this->schoolId();
        $campaigns = $this->campaignQuery()
            ...
            ->when($this->historySearch !== '', function (Builder $query): void {
                ...
                $query->where(function (Builder $matches) use ($like): void {
                    $matches->where('subject', 'like', $like)
        ...
        $classes = SchoolClass::query()
            ...
            ->where('status', 'active')
            ->whereHas('academicYear', fn (Builder $years) => $years
        ...
        $stats = [
            ...
            'processing' => $this->campaignQuery()->whereIn('status', [EmailCampaign::STATUS_QUEUED, EmailCampaign::STATUS_PROCESSING])->count(),
            'failed' => (int) $this->campaignQuery()->sum('failed_count'),
        ];

        return view('livewire.lms.emails.index', [
            'campaigns' => $campaigns,
            'classes' => $classes,
    ...
    private function validateComposer(): array
    {
        return $this->validate([
            'mode' => ['required', Rule::in(['single', 'bulk'])],
            'audiences' => [Rule::requiredIf($this->mode === 'bulk'), 'array', 'max:3'],
            'audiences.*' => [Rule::in(EmailRecipientResolver::AUDIENCES)],
    ...
    {
        return EmailCampaign::query()->where('school_id', $this->schoolId());
    }
    ...
    private function schoolId(): int
    {
        ...
        abort_unless($schoolId, 404, 'School setup is required before sending emails.');
