                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse ($assignments as $assignment)
                        <tr wire:key="assignment-{{ $assignment->id }}">
                            ...
                            <td class="px-5 py-4"><div class="flex justify-end gap-2">@if(auth()->user()->hasRole('teacher'))<a href="{{ route('lms.assignments.teacher.grade', $assignment) }}" class="inline-flex items-center rounded-lg bg-emerald-50 px-3 py-2 text-xs font-semibold text-emerald-700 hover:bg-emerald-100">Grade</a>@endif<x-ui.icon-button wire:click="edit({{ $assignment->id }})" icon="edit" label="Edit assignment" target="edit({{ $assignment->id }})" /><x-ui.icon-button wire:click="confirmDelete({{ $assignment->id }})" icon="trash" label="Archive assignment" variant="danger" target="confirmDelete({{ $assignment->id }})" /></div></td>
                        </tr>
                    @empty
