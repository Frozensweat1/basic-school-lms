<div class="space-y-6">
    <div class="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div>
            ...
            <h2 class="mt-2 text-2xl font-bold text-slate-900">My attendance</h2>
            <p class="mt-1 text-sm text-slate-600">Review your daily attendance and current attendance rate.</p>
        </div>
    ...
    <section class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        <div class="flex flex-col justify-between gap-3 sm:flex-row sm:items-end">
            <div class="grid gap-3 sm:grid-cols-2">
                <div>
                    <label for="student-attendance-from" class="block text-sm font-medium text-slate-700">From</label>
                    <input wire:model.live="fromDate" id="student-attendance-from" type="date" max="{{ now()->toDateString() }}" class="mt-1 rounded-xl border-slate-300 text-sm shadow-sm focus:border-blue-700 focus:ring-blue-700">
                    @error('fromDate') <p class="mt-1 text-sm text-rose-700">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label for="student-attendance-to" class="block text-sm font-medium text-slate-700">To</label>
                    <input wire:model.live="toDate" id="student-attendance-to" type="date" max="{{ now()->toDateString() }}" class="mt-1 rounded-xl border-slate-300 text-sm shadow-sm focus:border-blue-700 focus:ring-blue-700">
                    @error('toDate') <p class="mt-1 text-sm text-rose-700">{{ $message }}</p> @enderror
                </div>
            </div>
            <span wire:loading wire:target="fromDate,toDate" class="pb-1 text-xs font-medium text-slate-500">Refreshing attendance…</span>
        </div>
    </section>
            ...
            <table class="min-w-full divide-y divide-slate-200 text-sm">
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse ($records as $record)
                        ...
                        @endphp
                        <tr wire:key="student-attendance-{{ $record->id }}" class="hover:bg-slate-50/80">
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="px-5 py-14 text-center text-slate-500">No attendance records match this date range.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
