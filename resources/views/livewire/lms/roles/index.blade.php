<div class="space-y-6">
    <div class="flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:flex-row sm:items-center sm:justify-between">
        ...
        <x-button variant="primary" size="md">Add role</x-button>
    </div>

    <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <x-empty-state
            title="No roles configured"
            description="Define roles and assign the system permissions they need."
            :action="'<x-button variant=\'primary\'>Create role</x-button>'"
        />
    </div>
</div>
