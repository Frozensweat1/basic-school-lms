...
<a href="{{ $href }}" wire:navigate @if ($active) aria-current="page" @endif
    {{ $attributes->class([
        ...
        'text-[var(--brand-primary)] after:absolute after:inset-x-2 after:bottom-0 after:h-0.5 after:rounded-full after:bg-[var(--brand-accent)] relative' =>
            $active && !$mobile,
    ...
    <span>{{ $slot }}</span>
    @if ($mobile)
        <svg aria-hidden="true" class="h-4 w-4 opacity-60" fill="none" viewBox="0 0 24 24" stroke="currentColor"
            stroke-width="2">
